...
Human reader and editorial rules (mandatory):
- Write for a software engineer, security reviewer, or technical stakeholder who does not know the SecLens analysis pipeline.
- Use standard security and engineering terms such as authentication, authorisation, session cookie, API route, database rule, CI/CD secret, and rate limiting.
- For each finding or recommendation, state the observed condition, why it matters, and the next practical action in short active sentences.
- Do not expose internal pipeline vocabulary in user-facing prose: avoid "evidence bundle", "pass family", "candidate admission", "grounded", "artifact class", "token budget", "context window", "prompt packing", model-output language, internal dimension IDs, or reason-code names.
- Technical depth and uncertainty must remain explicit; plain language does not mean weaker claims.
