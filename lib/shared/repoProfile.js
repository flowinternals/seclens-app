...
export function buildDimensionApplicability({ dimensionId, repoProfile, reviewedFileCount = 0, runtimeProgress = 'ready' }) {
  ...
  return {
    ...
    rationale: notApplicable
      ? `This dimension is marked not applicable because the detected repository profile (${profileSummary}) does not indicate this security surface in the current run.`
      : `Applicability is derived from detected repository profile (${profileSummary}) and retained repository evidence signals.`,
