...
export function extractOpeningProseFromMarkdown(raw, opts = {}) {
  ...
  while (paragraphs.length < maxParagraphs && i < lines.length) {
    const para = []
    ...
    if (para.length) paragraphs.push(para.join(' '))
      ...
      // Hit a ## section (or EOF) with no prose buffered — stop. Advancing is required to
      // avoid an infinite outer loop when i still points at the heading line.
      break
    }
