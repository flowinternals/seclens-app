...
/** Shown as 0% until a dimension finishes and applicability is computed for the run. */
export const SCAN_PENDING_APPLICABILITY = Object.freeze({
  status: 'still_working',
  weight: 0,
  rationale: 'Applicability updates when each dimension completes.',
  required: true,
})
...
function recommendationQueueDedupeKey(item) {
  return `${item.dimensionId}:${item.text}:${item.evidenceTarget}`
}

/** Queue rows from findings / unverified controls when the model leaves `recommendations` empty. */
function fallbackRecommendationQueueItems(dimensions) {
  const out = []
  for (const dimension of dimensions || []) {
    const dimId = dimension.dimensionId
    ...
    for (const finding of dimension.findings || []) {
      const text = String(finding.claim || finding.title || '').trim()
      if (!text) continue
      fi += 1
      out.push({
        id: finding.id || `${dimId}-queue-finding-${fi}`,
        dimensionId: dimId,
        dimensionLabel: label,
        priority: finding.severity || finding.priority || 'medium',
        text,
        evidenceTarget:
          (Array.isArray(finding.evidence_citations) && finding.evidence_citations[0]) ||
          finding.evidenceTarget ||
          defaultEvidence,
        confidence: finding.confidence || dimension.coverage?.confidence,
      })
    }

    let ui = 0
    for (const u of dimension.unverifiedControls || []) {
      const text = String(u.claim || u.title || '').trim()
      if (!text) continue
      ui += 1
      const conf = normalizePriorityText(u.confidence)
      out.push({
        id: u.id || `${dimId}-queue-uv-${ui}`,
        dimensionId: dimId,
        dimensionLabel: label,
        priority: conf.includes('high') ? 'high' : 'medium',
        text,
        evidenceTarget: (Array.isArray(u.evidence_citations) && u.evidence_citations[0]) || defaultEvidence,
        confidence: u.confidence || dimension.coverage?.confidence,
      })
    }
  }
  return out
...
export function summarizeDashboard(dimensions) {
  ...
  const totals = list.reduce(
    (acc, dimension) => {
      ...
      acc.dimensionsPlanned += 1
      const applicability = dimension?.applicability?.status
      if (applicability !== 'not_applicable') {
        acc.dimensionsApplicable += 1
      }
      const reviewedPathCount =
        (Array.isArray(dimension?.evidence?.reviewedPaths)
          ? dimension.evidence.reviewedPaths.filter(Boolean).length
          : 0) || Number(dimension?.coverage?.reviewedFiles || 0)
      // AQ7: dimensionsReviewed = model-examined with ≥1 path (not mere partial/completed progress).
      if (reviewedPathCount > 0) {
        acc.dimensionsReviewed += 1
      }
      if (dimension.progress === 'completed') {
        acc.dimensionsCompleted += 1
      }
      ...
      // Dimension-level counts stay on each dimension; run-level reviewedFiles is unique paths (CR-012 Q3).
    ...
    },
    {
      totalDimensions: 0,
      dimensionsPlanned: 0,
      dimensionsApplicable: 0,
      dimensionsSurfaced: 0,
      dimensionsAssigned: 0,
  ...
  totals.reviewedFiles = reviewedPathSet.size
  const pathDimensionHits = new Map()
  for (const dimension of list) {
    for (const path of dimension?.evidence?.reviewedPaths || []) {
      if (!path) continue
      pathDimensionHits.set(path, (pathDimensionHits.get(path) || 0) + 1)
    }
  }
  let multiAssignedPathCount = 0
  for (const n of pathDimensionHits.values()) {
    if (n > 1) multiAssignedPathCount += 1
  }
  totals.multiAssignedPathCount = multiAssignedPathCount
  ...
  const fromModelRecommendations = list.flatMap((dimension) =>
  ...
  const recommendationQueue = uniqueBy(
    [...fromModelRecommendations, ...fallbackRecommendationQueueItems(list)],
    recommendationQueueDedupeKey
  ).sort((left, right) => {
  ...
  let recommendationsHigh = 0
  let recommendationsMedium = 0
  let recommendationsLow = 0
  const recommendationEvidencePaths = new Set()
  for (const item of recommendationQueue) {
    const basis = normalizePriorityText(item?.priority || item?.severity)
    if (basis.includes('critical') || basis.includes('p0') || basis.includes('high')) {
      recommendationsHigh += 1
    } else if (basis.includes('low') || basis.includes('p2')) {
      recommendationsLow += 1
    } else {
      recommendationsMedium += 1
    }
    const path = pathFromCitation(String(item.evidenceTarget || ''))
    if (path) recommendationEvidencePaths.add(path)
  }
  totals.recommendationsHigh = recommendationsHigh
  totals.recommendationsMedium = recommendationsMedium
  totals.recommendationsLow = recommendationsLow
  totals.filesWithRecommendationEvidence = recommendationEvidencePaths.size
  ...
  // AQ7: export readiness is about terminal progress, not model-examined count.
  // Zero-path Incomplete (NO_PASS_EVIDENCE) is a finished coverage outcome and must not block export.
  const unfinished =
    (totals.progressDistribution.queued || 0) +
    (totals.progressDistribution.reviewing || 0) +
    (totals.progressDistribution.synthesizing || 0)
  if (unfinished > 0) {
  ...
  if (totals.progressDistribution.failed > 0) {
    reportReadinessReasons.push(
      'One or more dimensions hit a model or runtime error (distinct from a no-evidence coverage skip) and must complete successfully before export.'
    )
  }
  ...
  const applicableIncompleteCount = list.filter(
    (dimension) =>
      dimension?.applicability?.status !== 'not_applicable' &&
      dimension?.applicability?.required !== false &&
      dimension?.progress !== 'completed'
  ).length

  // DEFECT-004: never treat incomplete applicable dimensions as a fully healthy launch posture,
  // especially when the run admitted zero findings (avoids "Ready to launch/caution" overconfidence).
  if (totals.progressDistribution.failed > 0 && overallStatus === 'healthy') {
    overallStatus = 'attention'
  }
  if (applicableIncompleteCount > 0 && totals.findingsAdmitted === 0) {
    if (overallStatus === 'healthy' || overallStatus === 'attention') {
      overallStatus = 'unknown'
    }
  } else if (applicableIncompleteCount > 0 && overallStatus === 'healthy') {
    overallStatus = 'attention'
  }

  // DEFECT-004 / AQ7: if model-examined count is below applicable dimensions, do not present launch-ready posture.
  if (
    totals.dimensionsApplicable > 0 &&
    totals.dimensionsReviewed < totals.dimensionsApplicable
  ) {
    if (overallStatus === 'healthy' || overallStatus === 'attention') {
      overallStatus = 'unknown'
    }
  } else if (totals.dimensionsReviewed < totals.totalDimensions) {
    if (overallStatus === 'healthy' || overallStatus === 'attention') {
      overallStatus = 'unknown'
    }
  }
...
export function createDashboardPayload({
  ...
  // Merge assignment lifecycle surfaced/assigned when present (AQ7).
  const lifecycle = telemetry?.assignment?.lifecycleCounts
  if (lifecycle && typeof lifecycle === 'object') {
    if (Number.isFinite(Number(lifecycle.surfaced))) {
      summary.totals.dimensionsSurfaced = Number(lifecycle.surfaced)
    }
    if (Number.isFinite(Number(lifecycle.assigned))) {
      summary.totals.dimensionsAssigned = Number(lifecycle.assigned)
    }
    if (Number.isFinite(Number(lifecycle.planned))) {
      summary.totals.dimensionsPlanned = Number(lifecycle.planned)
    }
  }
  // Keep reviewed/completed authoritative from dimension results; sync back into assignment lifecycle.
  if (telemetry?.assignment) {
    telemetry.assignment.lifecycleCounts = {
      ...(telemetry.assignment.lifecycleCounts || {}),
      planned: summary.totals.dimensionsPlanned,
      applicable: summary.totals.dimensionsApplicable,
      surfaced: summary.totals.dimensionsSurfaced,
      assigned: summary.totals.dimensionsAssigned,
      reviewed: summary.totals.dimensionsReviewed,
      completed: summary.totals.dimensionsCompleted,
    }
  }
...
export function createMockDashboardPayload() {
  ...
  return createDashboardPayload({
    ...
    completedAt: '2026-04-30T01:02:10.000Z',
