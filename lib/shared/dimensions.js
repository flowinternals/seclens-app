...
export function summarizeDashboard(dimensions) {
  ...
  const totals = list.reduce(
    (acc, dimension) => {
      ...
      if (dimension.progress === 'completed' || dimension.progress === 'partial') {
  ...
  // DEFECT-004: if the run did not count every planned dimension as reviewed (e.g. failed/queued rows),
  // summary language must not read as launch-ready - align posture with the executive "completed N of M" line.
  if (totals.dimensionsReviewed < totals.totalDimensions) {
    if (overallStatus === 'healthy' || overallStatus === 'attention') {
      overallStatus = 'unknown'
    }
