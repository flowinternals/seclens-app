...
import { buildSecuritySurfacePlan, evaluateDimensionCoreEvidence } from './securitySurfaceTargets.js'
...
export async function fetchRepositoryContent(repoUrl, options = {}) {
  try {
    ...
    const securitySurfacePlan = buildSecuritySurfacePlan(blobPaths, repoProfile, { maxFiles: caps.maxFiles })
    ...
    for (const path of seedTextPaths) {
      try {
        ...
        if (buf.includes(0) || text.includes('\uFFFD')) continue
    ...
    if (selectionPlan.protectedCoverageGap && process.env.SECLENS_FAIL_ON_PROTECTED_DROP === 'true') {
      throw new Error(
        'SecLens protected security targets were dropped by the file cap. Increase caps or reduce repository breadth; set SECLENS_FAIL_ON_PROTECTED_DROP=false to allow partial coverage.'
      )
    }
