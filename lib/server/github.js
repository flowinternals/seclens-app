...
export async function fetchRepositoryContent(repoUrl, options = {}) {
  const phaseLog = typeof options.onPhase === 'function' ? options.onPhase : null
  const mark = (phase, extra = {}) => {
    if (phaseLog) phaseLog(phase, extra)
    else if (process.env.SECLENS_FETCH_DEBUG === '1') {
      console.log(`[fetchRepositoryContent] ${phase}`, extra)
    }
  }
  ...
  try {
    ...
    const { owner, repo, explicitRef, ghRepo, fetchWithAuth, apiDefaultBranch, repoUrl: resolvedRepoUrl } = meta

    const selectedRef = explicitRef || apiDefaultBranch
    mark('resolve_ref_start')
    const refResult = await resolveScanRef(fetchWithAuth, owner, repo, selectedRef, {
      metadataResolved: !!selectedRef,
    })
    mark('resolve_ref_done', { sha: refResult?.sha?.slice?.(0, 7) })
    ...
    mark('tree_done', { truncated: apiTruncated, entries: Array.isArray(treePayload.tree) ? treePayload.tree.length : 0 })
    ...
    mark('paths_ready', { blobPaths: blobPaths.length, treeSizeCapHit })
    ...
    mark('docs_fetch_start', { count: documentationPaths.length })
    for (const p of documentationPaths) {
      await fetchPathTextIntoMap(p)
    }
    mark('docs_fetch_done')

    mark('infer_profile_start')
    const baseProfile = inferRepoProfileFromPaths(blobPaths, ghRepo.language || '')
    mark('infer_profile_done')
    ...
    mark('surface_plan_start')
    let securitySurfacePlan = buildSecuritySurfacePlan(blobPaths, repoProfile, surfacePlanOpts)
    mark('surface_plan_done', { critical: securitySurfacePlan.criticalShortlist?.length })
    ...
    mark('shortlist_text_fetch_start', { critical: securitySurfacePlan.criticalShortlist.length })
    ...
    mark('shortlist_text_fetch_done', { texts: Object.keys(pathTextByPath).length })

    const aliasAtRoots = detectAliasRootsFromConfigTexts(
      configPaths.map((p) => pathTextByPath[p]).filter(Boolean)
    )

    const keywordCandidates = pickBoundedKeywordScanCandidates(blobPaths, repoProfile, securitySurfacePlan, {
      max: 360,
    })
    mark('keyword_text_fetch_start', { candidates: keywordCandidates.length })
    for (const path of keywordCandidates) {
      if (shortlistSourceRe.test(path)) await fetchPathTextIntoMap(path)
    }
    mark('keyword_enrich_start', { texts: Object.keys(pathTextByPath).length })
    ...
    for (let r = 0; r < 8; r++) {
      ...
      mark('import_enrich_round_start', { round: r + 1, critical: securitySurfacePlan.criticalShortlist.length })
      ...
      enrichRounds = r + 1
      mark('import_enrich_round_done', {
        round: enrichRounds,
        critical: securitySurfacePlan.criticalShortlist.length,
        texts: Object.keys(pathTextByPath).length,
      })
    ...
    mark('final_select_done', { selected: selectionPlan.selected?.length })
    ...
    mark('contents_fetch_start', { files: metaList.length, batchSize })
    ...
    for (let batchStart = 0; batchStart < metaList.length; batchStart += batchSize) {
      ...
      if (batchStart === 0 || batchStart % (batchSize * 5) === 0) {
        mark('contents_fetch_batch', { at: batchStart, of: metaList.length })
      }
    ...
    mark('contents_fetch_done', { orderedFiles: orderedFiles.length, omissions: fetchOmissions.length })
