...
function buildRepoContext(repoProfile) {
  ...
  return parts.join('. ') || 'Repository stack and layout detected from selected file paths.'
}
...
export function buildAiIdePromptFromRecommendation({
  ...
  const promptBody = [
    `Task:`,
    `Perform a targeted security review for ${dimensionLabel}.`,
    ``,
    `Target files:`,
    ...targetFiles.map((f) => `- ${f}`),
    ``,
...
export function validateAiIdePrompt(prompt, dimensionId) {
  ...
  // DEFECT-OUTPUT-001: do not require raw dimensionId snake_case in customer-facing prompt body.
  // Specificity is enforced via structured dimensionId field + non-generic body heuristics above.
