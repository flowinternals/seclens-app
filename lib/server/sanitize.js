...
/**
 * Strip C0 controls + space (U+0000–U+0020) and NBSP (U+00A0) from a scheme token.
 * Character-wise (no control-char regex) so null-byte / whitespace obfuscation still collapses
 * e.g. `java\0script` → `javascript` without triggering ESLint `no-control-regex`.
 * @param {string} token
 * @returns {string}
 */
function stripSchemeTokenNoise(token) {
  let out = ''
...
function normalizeSchemeTokenBeforeColon(decoded) {
  ...
  return stripSchemeTokenNoise(decoded.slice(0, colon)).toLowerCase()
}
...
function stripInlineEventHandlerAttributesOnce(markdown) {
  let out = ''
  let i = 0
  while (i < markdown.length) {
...
/**
 * Repeat until stable so nested/obfuscated on* attributes cannot survive a single pass
...
function stripInlineEventHandlerAttributes(markdown) {
  ...
  let guard = 0
  while (guard++ < 32) {
    const next = stripInlineEventHandlerAttributesOnce(cur)
    if (next === cur) break
    cur = next
  }
  return cur
