...
import { resolvePrimaryPartialReason } from './coverageHonesty.js'
...
function buildCoverageSummary(label, runtime, reviewedFiles, omittedFilesRelevant, capLimited) {
  ...
  if (reviewedFiles === 0) {
    return `No directly relevant files were kept for ${label.toLowerCase()}, so this dimension requires expanded review before launch sign-off.`
  }
  ...
  if (capLimited || omittedFilesRelevant > 0) {
    // DEFECT-OUTPUT-001 / plan Q3 — human-facing coverage omission wording
    coverageParts.push(
      'this area could not be assessed fully because the scan did not include enough relevant files'
    )
  }
  ...
  const whatRemainsUnclear =
    topFinding
      ...
      : topUnverified
        ...
        : capLimited
          ? 'This area could not be assessed fully because the scan did not include enough relevant files, so this dimension needs an expanded review before launch sign-off.'
          : reviewedCount === 0
    ...
    topRecommendation
      ...
      : topFinding
        ...
        : topUnverified
          ...
          : capLimited
            ? 'Re-run with broader file retention or review omitted files manually before signing this dimension off.'
