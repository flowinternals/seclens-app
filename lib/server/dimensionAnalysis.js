...
import { escapeMarkdownTableCell } from './markdownTableEscape.js'
import {
  resolvePrimaryPartialReason,
  buildAllocationLedgerExportSection,
  HOTSPOT_SCOPED_CLAIM,
...
function buildCoverageSummary(label, runtime, reviewedFiles, omittedFilesRelevant, capLimited, concreteOmitSample = []) {
  ...
  if (reviewedFiles === 0) {
    return `No directly relevant files were model-examined for ${label.toLowerCase()} (reason: no pass evidence).`
  }

  const coverageParts = [`Model-examined ${pluralize(reviewedFiles, 'file')} directly related to ${label.toLowerCase()}`]
  if (capLimited || omittedFilesRelevant > 0) {
    const sample = concreteOmitSample.filter(Boolean).slice(0, 3)
    if (sample.length > 0) {
      coverageParts.push(
        `${omittedFilesRelevant || sample.length} path(s) omitted by cap/quota (e.g. ${sample.join(', ')})`
      )
    } else if (omittedFilesRelevant > 0) {
      coverageParts.push(`${omittedFilesRelevant} path(s) omitted by cap or per-dimension quota`)
    } else {
      coverageParts.push('coverage was limited by an ingestion or prompt cap')
    }
  }
...
function buildSummary({
  ...
  const whatWasReviewed =
    runtime?.progress === 'failed'
      ? `The ${label.toLowerCase()} review did not finish cleanly, so its results should be treated as incomplete.`
      : reviewedCount > 0
        ? `SecLens reviewed ${pluralize(reviewedCount, 'path')} for ${label.toLowerCase()} and used the strongest supporting file references.`
        : `SecLens did not keep directly relevant reviewed paths for ${label.toLowerCase()} in this run.`

  const whatLooksStrong =
    topObserved
      ? controlLine(topObserved, 'At least one control looked present in the reviewed files.')
      : findings.length === 0 && unverifiedControls.length === 0 && reviewedCount > 0
        ? `The reviewed files did not surface an obvious break in ${label.toLowerCase()}, but that is not the same as a hard security guarantee.`
        : 'No control was strong enough to call confirmed from the reviewed files alone.'

  const whatRemainsUnclear =
    topFinding
      ? controlLine(topFinding, 'A concrete issue needs follow-up.')
      : topUnverified
        ? controlLine(topUnverified, 'One or more expected controls could not be confirmed from the reviewed files.')
        : capLimited
          ? 'Coverage for this dimension was limited by a cap or quota; omitted paths are listed in the coverage allocation section when available.'
          : reviewedCount === 0
            ? `This dimension needs expanded file coverage because SecLens did not keep directly relevant files for ${label.toLowerCase()}.`
  ...
  const whatToCheckNext =
    topRecommendation
      ...
      : topFinding
        ...
        : topUnverified
          ? 'Manually verify the control described above in the cited files before treating this dimension as healthy.'
          : capLimited
            ? 'Review omitted paths for this dimension when they are listed in coverage allocation, then re-run if release-critical.'
            : 'Use the referenced files as a spot-check sample before treating this dimension as complete.'
...
export function assembleDimensionResult({
  ...
  runtime = {},
  coverage = {},
  unmatchedSurfacedCount = 0,
  omittedByQuotaCount = 0,
  reasonCode: reasonCodeOverride = null,
}) {
  ...
  const omittedFilesRelevant = Math.max(
    0,
    Number(promptTrimmedEvidenceCount || 0) + Number(omittedByQuotaCount || 0)
  )
  const capLimited =
    ...
    Number(promptTrimmedEvidenceCount || 0) > 0
  ...
  let progress = runtime.progress || 'completed'
  if (
    progress === 'completed' &&
    (Number(unmatchedSurfacedCount || 0) > 0 || Number(omittedByQuotaCount || 0) > 0)
  ) {
    progress = 'partial'
  }

  const parseError =
    Boolean(runtime.parseError) ||
    (progress === 'partial' &&
      !capLimited &&
      Number(omittedByQuotaCount || 0) <= 0 &&
      Number(unmatchedSurfacedCount || 0) <= 0 &&
      Number(promptTrimmedEvidenceCount || 0) <= 0 &&
      reviewedFileCount > 0)

  const reasonCode =
    reasonCodeOverride ||
    (progress === 'partial' || progress === 'failed'
      ? resolvePrimaryPartialReason({
          parseError,
          promptTrimmedCount: promptTrimmedEvidenceCount,
  ...
  return createEmptyDimensionResult(dimensionId, {
    ...
    coverage: {
      ...
      confidence,
      coverageSummary,
      unmatchedSurfacedCount: Number(unmatchedSurfacedCount || 0),
      omittedByQuotaCount: Number(omittedByQuotaCount || 0),
      maxFilesCapHit: Boolean(coverage?.maxFilesCapHit),
      maxBytesPerFileCapHit: Boolean(coverage?.maxBytesPerFileCapHit),
      maxTotalBytesCapHit: Boolean(coverage?.maxTotalBytesCapHit),
      maxTreeSizeCapHit: Boolean(coverage?.maxTreeSizeCapHit),
    },
    evidence: {
      topCitations,
...
export function assembleSkippedDimensionResult(dimensionId, reason = 'no_relevant_evidence', repoProfile = null) {
  ...
  // Dimension status shows Incomplete (AQ7). Export readiness uses terminal progress, not model-examined count.
  return createEmptyDimensionResult(dimensionId, {
    status: isNotApplicable ? 'healthy' : 'unknown',
    progress: isNotApplicable ? 'completed' : isMissingEvidence ? 'partial' : 'completed',
    reasonCode: isMissingEvidence ? 'NO_PASS_EVIDENCE' : null,
    coverage: {
      reviewedFiles: 0,
    ...
    summary: {
      ...
      whatLooksStrong: isNotApplicable
        ...
        : 'No control was strong enough to call confirmed without directly relevant reviewed files.',
      whatRemainsUnclear: isNotApplicable
        ...
        : `This dimension requires expanded file coverage before launch sign-off for ${definition.label.toLowerCase()}.`,
...
- **Project purpose (from docs when available):** ${repoProfile.applicationPurpose || repoProfile.profileSummary || 'Not extracted for this run.'}
- **Layout certainty:** ${repoProfile.architectureConfidence || repoProfile.confidence || 'unknown'} (structural path patterns)
- **Toolchain certainty:** ${repoProfile.stackConfidence || 'unknown'} (package.json, docs, and manifest paths)
- **Overall (conservative):** ${repoProfile.confidence || 'unknown'}
- **Architecture (path inventory):** ${repoProfile.rationale || 'No path inventory rationale was generated for this run.'}
...
${applicabilityTable}`
  ...
  const modelExamined = totals.dimensionsReviewed || 0
  const applicable = totals.dimensionsApplicable || dimensions.length
  const completed = totals.dimensionsCompleted || 0
  const planned = totals.dimensionsPlanned || totals.totalDimensions || dimensions.length
  const executiveSection = `## Executive Posture Summary

SecLens model-examined ${modelExamined} of ${applicable} applicable security dimensions in this run (${completed} completed of ${planned} planned).
...
${HOTSPOT_SCOPED_CLAIM}`
  ...
  const allocationSection = buildAllocationLedgerExportSection(
    dashboard?.telemetry?.assignment?.allocationLedger || []
  )
  const capNote = dashboard?.telemetry?.capProvenanceNote
    ? `\n\n${dashboard.telemetry.capProvenanceNote}`
    : ''
  const honestyCallout = dashboard?.telemetry?.coverageHonestyCallout
    ? `\n\n${dashboard.telemetry.coverageHonestyCallout}`
    : ''
  const coverageFactsSection = [allocationSection, capNote, honestyCallout].filter(Boolean).join('\n') || null
...
Strengths supported by the reviewed files and references.
...
${toArray(dashboard?.recommendationQueue)
  ...
  const confidenceSection = `## Coverage & recommendation severity

Scope and prioritized hardening suggestions from this analysis:

- **Reviewed files (counted once per path):** ${summary?.totals?.reviewedFiles || 0}
- **Files tied to at least one recommendation (by evidence path):** ${summary?.totals?.filesWithRecommendationEvidence ?? 0}
- **High-severity hardening recommendations:** ${summary?.totals?.recommendationsHigh ?? 0}
- **Medium-severity hardening recommendations:** ${summary?.totals?.recommendationsMedium ?? 0}
- **Lower-severity hardening recommendations:** ${summary?.totals?.recommendationsLow ?? 0}

Severity labels prioritize where to invest effort next; they describe suggested hardening work, not a formal risk rating of the entire repository.`
...
export function buildRepositoryDisplay(repositoryUrl, fetchedRepository = null) {
  if (fetchedRepository) {
    return {
      ...
      description:
        typeof fetchedRepository.description === 'string' && fetchedRepository.description.trim()
          ? fetchedRepository.description.trim()
          : null,
