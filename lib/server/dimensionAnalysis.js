...
function escapeMarkdownTableCell(value) {
  // Escape backslashes before pipes so `\|` cannot break out of cells (CodeQL js/incomplete-sanitization).
  return String(value ?? '')
    .replace(/\\/g, '\\\\')
    .replace(/\|/g, '\\|')
    .replace(/\r?\n+/g, ' ')
...
export function assembleDimensionResult({
  ...
  return createEmptyDimensionResult(dimensionId, {
    status,
    progress: runtime.progress || 'completed',
    findings,
    observedControls,
    unverifiedControls,
    recommendations,
    ...
    applicability: buildDimensionApplicability({
      ...
      repoProfile,
      reviewedFileCount,
      runtimeProgress: runtime.progress || 'completed',
    }),
