 ...
 */
function injectInspectedSurfaceObservations(admitted, bundle, rejections) {
        ...
        if (hasCitation && citationIntegrityOk) {
          const scopeReason =
            failReason === 'severity_calibration_mismatch' || failReason === 'insufficient_evidence_pack'
              ? 'hedge_or_pack'
              : 'structure'
          const obs = buildDownscopedObservationFromFinding(c, scopeReason)
          const sig = `observation|${obs.topic}|${obs.title.toLowerCase()}|${obs.claim.toLowerCase()}`
          if (!seenSignatures.has(sig)) {
            seenSignatures.add(sig)
            admitted.observations.push(obs)
            rejections.push({
              candidate_id: c.candidate_id,
              reason: `downscoped_to_observation:${failReason}`,
            })
          } else {
...
function computeCr009ValueParts(ctx) {
  ...
  let appendixUse = 0
  if (appendixEvidenceCount < 8) appendixUse = appendixPlan?.renderedCount > 0 ? 0.5 : 0
  else if (appendixPlan && appendixPlan.renderedCount > 0) appendixUse = 1.0
  else appendixUse = 0
