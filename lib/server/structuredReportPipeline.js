...
import { HUMAN_DOWNSCOPE_WEAKNESS, HUMAN_EVIDENCE_STANDARD_NOT_MET } from './humanReadableOutput.js'
...
function isMetaNonSecurityCandidate(candidate) {
  ...
  for (const cite of cites) {
    ...
    if (!path) continue
    const isReportQualityPath = /(?:^|[/\\])tests[/\\]report-quality[/\\]/i.test(path)
    const isReportValidationTest = /reportvalidation\.test\./i.test(path)
    const isUnitTestFile = /[/\\][^/\\]+\.test\.(?:js|ts|tsx)$/i.test(path)
    const isUnderTestsTree = /(?:^|[/\\])tests[/\\].*\.(?:js|ts|tsx)$/i.test(path)
    if (isReportQualityPath || isReportValidationTest || isUnitTestFile || isUnderTestsTree) {
...
function buildDownscopedObservationFromFinding(candidate, reason) {
  const citeRaw = candidate.evidence_citations?.[0]
  const citeLabel = citeRaw ? `\`${citeRaw}\`` : 'the reviewed files'
  const topicHint = candidate.topic ? String(candidate.topic).replace(/_/g, ' ') : 'the relevant control area'
  /** DEFECT-OUTPUT-001: human-facing downscope wording (plan Q3). */
  const claim =
    reason === 'hedge_or_pack'
      ? `${HUMAN_DOWNSCOPE_WEAKNESS} In ${citeLabel}, targeted validation of ${topicHint} is still recommended.`
      : `${HUMAN_EVIDENCE_STANDARD_NOT_MET} In ${citeLabel}, targeted validation of ${topicHint} is still recommended.`
  return {
...
function buildDownscopedNonFindingFromCandidate(candidate, kind, reason) {
  ...
  const reasonNote =
    kind === 'recommendation'
      ? 'the reviewed files support hardening guidance but not a confirmed security issue'
      : 'an important control could not be conclusively proven from the reviewed files in this run'
  return {
    claimSchemaVersion: candidate.claimSchemaVersion || 1,
    ...
    claim:
      kind === 'recommendation'
        ? `In ${citeLabel}, perform targeted hardening for ${topicHint}; ${reasonNote}.`
        : `In ${citeLabel}, verify ${topicHint} controls before reporting a confirmed finding; ${reasonNote}.`,
    specific_code_behavior: candidate.specific_code_behavior,
    missing_control_or_unsafe_condition: candidate.missing_control_or_unsafe_condition,
    impact: candidate.impact,
    evidence_citations: [...candidate.evidence_citations],
