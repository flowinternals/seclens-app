...
export function classifySelectionDomain(normPath) {
  ...
  if (
    /(^|\/)firestore\.rules$|(^|\/)storage\.rules$|(^|\/)firebase\.json$|\.gitleaks|pre-commit|dependabot|renovate\.|(^|\/)security\.md$/i.test(
      path
    ) ||
...
export function selectPathsByTiers(blobPaths, maxFiles, opts = {}) {
  ...
  for (const d of DOMAIN_RESERVE_ORDER) {
    buckets.get(d).sort((a, b) => {
      const ta = classifyRepoPath(a).tier || 3
      const tb = classifyRepoPath(b).tier || 3
  ...
  while (domainPhaseSlots > 0 && selected.length < caps && domainRoundProgress) {
    ...
    for (const d of DOMAIN_RESERVE_ORDER) {
      ...
      if (!next) continue
      const tierNum = classifyRepoPath(next).tier || 3
  ...
  const authInviteAnchorPresent = anchorMeta.some((a) =>
    /(^functions\/src\/|\/)(createuserandinvite|invitemanagement|validateinvite|updateuserclaims|index)\.(ts|js|mjs|cjs)$/i.test(
      a.path
    )
  )
  ...
  for (const c of candidates) {
    ...
    const t = classifyRepoPath(c.path).tier || 3
