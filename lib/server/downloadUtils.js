/**
 * Shared utilities for download endpoints (markdown, text, pdf)
 */

import { sanitizeMarkdown, sanitizeText } from './sanitize.js'

export const MANDATORY_EXPORT_DISCLAIMER =
  'SecLens is a repository security advisory tool. It does not prove the presence or absence of vulnerabilities. Recommendations, prompts and suggested tests are based on selected repository files and should be reviewed by a developer or security engineer before changes are made.'

const DISCLAIMER_SECTION_HEADING = '## Disclaimer'

export function appendMandatoryDisclaimer(markdown) {
  const base = typeof markdown === 'string' ? markdown.trim() : ''
  if (!base) {
    return `${DISCLAIMER_SECTION_HEADING}\n\n${MANDATORY_EXPORT_DISCLAIMER}`
  }
  if (base.includes(MANDATORY_EXPORT_DISCLAIMER)) {
    return base
  }
  return `${base}\n\n${DISCLAIMER_SECTION_HEADING}\n\n${MANDATORY_EXPORT_DISCLAIMER}`
}

/**
 * Generates a safe filename with project prefix and date stamp
 */
export function generateFilename(extension, repositoryName = 'report') {
  const timestamp = new Date().toISOString().slice(0, 10).replace(/-/g, '')
  const base = (repositoryName || 'report').toString()
  const sanitizedName = base.replace(/[^a-z0-9]/gi, '_').toLowerCase()
  return `SecLens_${sanitizedName}_${timestamp}.${extension}`
}

/**
 * Sets common headers for file downloads
 */
export function setDownloadHeaders(res, contentType, filename) {
  res.setHeader('Content-Type', contentType)
  res.setHeader('Content-Disposition', `attachment; filename="${filename}"`)
  res.setHeader('X-Content-Type-Options', 'nosniff')
}

/**
 * Safely prepare markdown content for download
 */
export function prepareMarkdown(markdown) {
  return sanitizeMarkdown(appendMandatoryDisclaimer(markdown))
}

/**
 * Convert markdown to plaintext and sanitize for text/pdf outputs
 */
export function markdownToPlainText(markdown) {
  if (!markdown || typeof markdown !== 'string') return ''
  const plain = markdown
    .replace(/^#+\s+/gm, '')
    .replace(/\*\*(.*?)\*\*/g, '$1')
    .replace(/\*(.*?)\*/g, '$1')
    .replace(/`(.*?)`/g, '$1')
    .replace(/```[\s\S]*?```/g, '')
    .replace(/\[([^\]]+)\]\([^)]*\)/g, '$1')
    .replace(/^-\s+/gm, '- ')
    .replace(/^\d+\.\s+/gm, '')
  // `sanitizeText` is correct for HTML-facing output, but PDF/TXT are already
  // plain-text sinks. Decode the entities it creates so customers do not see
  // implementation escaping such as `&lt;` or `&amp;` in exported reports.
  return decodePlainTextEntities(decodePlainTextEntities(sanitizeText(plain)))
}

function decodePlainTextEntities(text) {
  return text
    .replace(/&amp;/g, '&')
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>')
    .replace(/&quot;/g, '"')
    .replace(/&#x27;/gi, "'")
}

const PDF_EXPORT_BLOCKERS = [
  {
    code: 'PDF_INTERNAL_REGISTER',
    pattern: /\b(?:evidencePaths|post-mortem|telemetry-only|pipeline register|advisory contract|FILE_[A-Z_]+|DIMENSION_[A-Z_]+)\b/i,
    message: 'PDF contains internal pipeline terminology.',
  },
  {
    code: 'PDF_WEAK_COVERAGE_DISCLAIMER',
    pattern: /this area could not be assessed fully because the scan did not include enough relevant files|re-run with broader file retention|remain outside this review|not enough relevant files|prioritised the highest-signal security surfaces/i,
    message: 'PDF contains a non-actionable or pathless coverage disclaimer.',
  },
  {
    code: 'PDF_HTML_ENTITY_LEAK',
    pattern: /&(?:lt|gt|amp|quot|#x27);/i,
    message: 'PDF contains escaped HTML entities instead of readable text.',
  },
]

/**
 * Validate the exact plaintext consumed by the PDF renderer. This is shared by
 * post-mortem and download endpoints so the quality gate covers the customer
 * export path, not only the dashboard/advisory JSON contract.
 */
export function validatePdfExportText(reportMarkdown) {
  const text = markdownToPlainText(appendMandatoryDisclaimer(reportMarkdown))
  const issues = PDF_EXPORT_BLOCKERS
    .filter(({ pattern }) => pattern.test(text))
    .map(({ code, message }) => ({ code, message }))
  return { valid: issues.length === 0, text, issues }
}

/**
 * Standardized error handler for download endpoints
 */
export function handleDownloadError(res, context, error) {
  const isDev = process.env.NODE_ENV === 'development'
  const message = context ? `${context} failed` : 'Operation failed'
  if (isDev) {
    console.error(message + ':', error?.message)
  } else {
    console.error(message)
  }
  return res.status(500).json({ error: message, code: 'DOWNLOAD_GENERATION_FAILED' })
}


