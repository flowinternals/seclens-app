...
import { RUN_COST_STATUS, buildRunCost } from '../shared/runCost.js'
...
export function buildTelemetry(analysisResult = {}, repoData, startedAtMs) {
  ...
  const requestedModel = analysisResult.requestedAnalysisModel
    ...
    : null
  const usedModelForCost =
    typeof analysisResult.analysisModel === 'string' && analysisResult.analysisModel.trim()
      ? analysisResult.analysisModel.trim()
      : selectedModel
  const runCost = buildRunCost({
    totalUsage: total,
    modelId: usedModelForCost,
  ...
  return {
    ...
    runCost,
    estimatedCostUsd:
      runCost.status === RUN_COST_STATUS.ESTIMATED ? runCost.estimatedCostUsd : null,
