...
import { HOTSPOT_SCOPED_CLAIM } from './coverageHonesty.js'
...
function toDimensionStatus(dimension) {
  ...
  if (progress === 'partial') {
    // Prefer dimension-provided primary reason (CR-012 Q1); never invent FILE_OMITTED_BY_CAP.
    const provided = String(dimension?.reasonCode || '').trim()
    if (provided) {
      return { status: 'WARNING', reasonCode: provided }
    }
    const coverage = dimension?.coverage || {}
    if (coverage.capLimited && Number(coverage.omittedFilesRelevant || 0) > 0) {
      return { status: 'WARNING', reasonCode: 'PROMPT_BUDGET_TRIM' }
    }
    if (coverage.capLimited) {
      return { status: 'WARNING', reasonCode: 'FILE_OMITTED_BY_CAP' }
    }
    if (Number(dimension?.coverage?.reviewedFiles || 0) <= 0) {
      return { status: 'WARNING', reasonCode: 'NO_PASS_EVIDENCE' }
    }
    return { status: 'WARNING', reasonCode: 'STRUCTURED_PARSE_PARTIAL' }
  }
  ...
  const dimensions = safeArray(dashboard?.dimensions).map((dimension) => {
    ...
    return {
      ...
      coverage: {
        ...
        coverageNotes: [
          String(dimension?.coverage?.coverageSummary || '').trim(),
...
export function validateAdvisoryOutputContract(contract) {
  ...
  // Plan Q4 hybrid D: rewrite dirty coverage notes, then hard-fail remaining customer prose.
  const { replacements: humanRegisterCoverageFallbacks } = applyHumanReadableCoverageFallbacks(contract)
  const humanRegisterIssues = lintHumanReadableAdvisory(contract)
  const { errors: humanRegisterErrors, warnings: humanRegisterWarnings } =
    partitionHumanRegisterIssues(humanRegisterIssues)
  if (humanRegisterErrors.length > 0) {
    errors.push(
      `AI-internal language found in user-facing advisory fields: ${humanRegisterErrors
  ...
  return {
    ok: errors.length === 0,
    errors,
    humanRegisterWarnings: [
      ...humanRegisterCoverageFallbacks.map((r) => ({
        code: r.code,
        path: r.path,
        action: 'coverage_fallback',
      })),
      ...humanRegisterWarnings,
    ],
  }
