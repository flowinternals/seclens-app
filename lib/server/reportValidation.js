...
/**
 * Executive "completed N of M" must not pair with optimistic summary risk (DEFECT-004).
 * @param {string} markdown
 */
export function hasConsolidatedExecutiveCompletenessMismatch(markdown) {
  ...
  const m = execBody.match(/SecLens completed (\d+) of (\d+) planned security dimensions/i)
  if (!m) return false
  const completed = Number.parseInt(m[1], 10)
  const total = Number.parseInt(m[2], 10)
  if (!Number.isFinite(completed) || !Number.isFinite(total) || completed >= total) return false
  const summaryLine = extractSummaryRiskLine(markdown) || ''
  const lower = summaryLine.toLowerCase()
