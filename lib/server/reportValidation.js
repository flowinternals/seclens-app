 ...
 * Categories: STRUCTURE, HEADER_BLOCK, FINDINGS_PATTERN, APPENDIX, LEAKAGE, COVERAGE, SEVERITY_EVIDENCE (DEFECT-001), MISLEADING_SECRET_CLASSIFICATION (DEFECT-003), SPECULATIVE_FINDING (DEFECT-004), UNBOUNDED_ABSENCE_CLAIM (DEFECT-006), SUMMARY_RISK_INCONSISTENT, NOT_EVIDENCED_DRIFT, UNKNOWN
 */

import { SECTION_TITLES_ORDER } from '../prompts/seclens-output-contract-v2.js'

/** @typedef {'STRUCTURE'|'HEADER_BLOCK'|'FINDINGS_PATTERN'|'APPENDIX'|'LEAKAGE'|'COVERAGE'|'SEVERITY_EVIDENCE'|'MISLEADING_SECRET_CLASSIFICATION'|'SPECULATIVE_FINDING'|'UNBOUNDED_ABSENCE_CLAIM'|'SUMMARY_RISK_INCONSISTENT'|'NOT_EVIDENCED_DRIFT'|'UNKNOWN'} ValidationCategory */
...
function evidenceContainsRepoPath(evidenceText) {
  ...
  return (
    /\b(api|src|server|tests|lib|app|pages|functions|packages|scripts|extensions|mcps|\.github)\/[\w./-]+\.(js|ts|jsx|tsx|json|mjs|cjs|ya?ml)\b/i.test(t) ||
    /[`'][\w./-]+\.(js|ts|jsx|tsx|json|mjs|cjs|ya?ml)[`']/i.test(t) ||
    /\b[\w.-]+\.(js|ts|jsx|tsx|json|mjs|cjs)(:\s*line\s+unknown|\s*:\s*\d+\s*-\s*\d+)?\b/i.test(t)
  )
}
...
export function hasSummaryRiskInconsistentWithFindings(markdown, keyFindingsBody) {
  ...
  if (summaryRank < 0 || findingsTopRank < 0) return false
