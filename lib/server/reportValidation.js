...
/**
 * Replace patterns that must not be echoed into the critic user prompt (Section13 A14).
...
/**
 * Normalize markdown heading titles for comparison (Section13 A9).
 * @param {string} raw
 */
...
/**
 * Consolidated dimension reports: header Summary Risk must not read "launch-ready" while dimension rows
 * show hard failures or broad coverage gaps (DEFECT-004 critic-style gate, evidence-bound).
 * @param {string} markdown
 */
export function hasConsolidatedSummaryPostureMismatch(markdown) {
  const summaryLine = extractSummaryRiskLine(markdown)
  if (!summaryLine) return false
  const lower = summaryLine.toLowerCase()
  const dimBody = extractSectionBody(markdown, 'Dimension Summaries')
  if (!dimBody?.trim()) return false

  const failed = /-\s*\*\*Progress:\*\*\s*failed\b/gi.test(dimBody)
  const partials = dimBody.match(/-\s*\*\*Progress:\*\*\s*partial\b/gi) || []
  const partialCount = partials.length
  const unknownOrStuck =
    /-\s*\*\*Progress:\*\*\s*(unknown|reviewing|queued|synthesizing)\b/gi.test(dimBody)

  if (lower.includes('ready to launch')) {
    return failed || partialCount > 0 || unknownOrStuck
  }
  if (lower.includes('ready with caution')) {
    if (failed) return true
    // A single evidence-limited or unresolved dimension is enough to contradict "caution" as sufficient.
    return partialCount >= 1 || unknownOrStuck
  }
  return false
}

/**
 * Executive model-examined / completed counts must not pair with optimistic summary risk (DEFECT-004 / AQ7).
 * @param {string} markdown
 */
export function hasConsolidatedExecutiveCompletenessMismatch(markdown) {
  const execBody = extractSectionBody(markdown, 'Executive Posture Summary')
  if (!execBody?.trim()) return false
  const legacy = execBody.match(/SecLens completed (\d+) of (\d+) planned security dimensions/i)
  const modern = execBody.match(
    /SecLens model-examined (\d+) of (\d+) applicable security dimensions[^\n]*\((\d+) completed of (\d+) planned\)/i
  )
  let examined
  let total
  if (modern) {
    examined = Number.parseInt(modern[1], 10)
    total = Number.parseInt(modern[2], 10)
  } else if (legacy) {
    examined = Number.parseInt(legacy[1], 10)
    total = Number.parseInt(legacy[2], 10)
  } else {
    return false
  }
  if (!Number.isFinite(examined) || !Number.isFinite(total) || examined >= total) return false
  const summaryLine = extractSummaryRiskLine(markdown) || ''
  const lower = summaryLine.toLowerCase()
  if (lower.includes('needs additional review') || lower.includes('not ready for launch')) return false
  return lower.includes('ready to launch') || lower.includes('ready with caution')
}
...
export function validateReport(markdown) {
  ...
  if (consolidatedStructureMatches && hasConsolidatedSummaryPostureMismatch(markdown)) {
    categories.push('SUMMARY_RISK_INCONSISTENT')
  }

  if (consolidatedStructureMatches && hasConsolidatedExecutiveCompletenessMismatch(markdown)) {
    categories.push('SUMMARY_RISK_INCONSISTENT')
  }
