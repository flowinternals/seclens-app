...
import { archiveSuccessfulScanHistory } from './scanHistoryStore.js'
...
function tryWriteScanJobLiveArtifact(record, extra = {}) {
  ...
  try {
    ...
    writeContainedUtf8Artifact(outDir, buildLiveArtifactFileName(record), JSON.stringify(safePayload, null, 2))
  } catch (err) {
...
function summarizeJob(record) {
  return {
    ...
    telemetry: record.telemetry,
    runCost: record.runCost || record.dashboard?.runCost || normalizeRunCost(null),
    requestedAnalysisModel: record.requestedAnalysisModel,
    analysisModel: record.analysisModel,
    triggeredBy: record.triggeredBy,
    error: record.error,
    historyArchive: record.historyArchive || null,
  }
}
...
function resolveCanonicalRunCost(analysisResult, fallbackModel, selectedModelId = null) {
  if (analysisResult?.runCost && typeof analysisResult.runCost === 'object') {
    return analysisResult.runCost
  }
  if (analysisResult?.dashboard?.runCost && typeof analysisResult.dashboard.runCost === 'object') {
    return analysisResult.dashboard.runCost
  }
  const total = analysisResult?.tokenUsage?.total || null
  const modelId = analysisResult?.analysisModel || fallbackModel || null
  return buildRunCost({
    totalUsage: total,
    modelId,
    selectedModelId: selectedModelId || analysisResult?.requestedAnalysisModel || null,
  })
}

function buildModelUsageSummary(analysisResult, fallbackModel, runCost = null) {
  const total = analysisResult?.tokenUsage?.total
  const telemetry = analysisResult?.dashboard?.telemetry
  if (!total && !telemetry && !runCost) return null
  const modelId = analysisResult?.analysisModel || fallbackModel || null
  const canonical = runCost || resolveCanonicalRunCost(analysisResult, fallbackModel)
  const estimatedCostUsd =
    canonical?.status === RUN_COST_STATUS.ESTIMATED ? canonical.estimatedCostUsd : null
  return {
    analysisModel: modelId,
    requestedAnalysisModel: analysisResult?.requestedAnalysisModel || null,
    promptTokens: total?.prompt_tokens ?? canonical?.inputTokens ?? null,
    completionTokens: total?.completion_tokens ?? canonical?.outputTokens ?? null,
    totalTokens: total?.total_tokens ?? canonical?.totalTokens ?? null,
    estimatedCostUsd,
    runCost: canonical,
  }
}
 ...
 */
function mergeTelemetryForLogEntry(telemetry, analysisResult, jobStartedAtMs, runCost = null) {
  const base = telemetry && typeof telemetry === 'object' ? { ...telemetry } : {}
  const tu = analysisResult?.tokenUsage
  const total = tu?.total
  const canonical = runCost || resolveCanonicalRunCost(analysisResult, base.analysisModel)
  if (canonical) {
    base.runCost = canonical
...
async function runScanJob({
  ...
  try {
    ...
    const usedModelId = analysisResult.analysisModel || record.analysisModel || analysisModel || null
    const selectedModelForCost = requestedAnalysisModel || record.requestedAnalysisModel || null
    const runCost = resolveCanonicalRunCost(
      {
        ...analysisResult,
        analysisModel: usedModelId,
        requestedAnalysisModel: selectedModelForCost,
      },
      usedModelId,
      selectedModelForCost
    )
    const reportWithCost = ensureRunCostStatementInReport(analysisResult.report, runCost)

    record.report = reportWithCost
    ...
    if (record.dashboard && typeof record.dashboard === 'object') {
      record.dashboard = {
        ...record.dashboard,
        runCost,
        report: typeof record.dashboard.report === 'string' ? reportWithCost : record.dashboard.report,
      }
    }
    ...
    record.telemetry =
      record.telemetry && typeof record.telemetry === 'object'
        ? { ...lifecycle, ...record.telemetry, runCost, estimatedCostUsd: runCost.status === RUN_COST_STATUS.ESTIMATED ? runCost.estimatedCostUsd : null }
        : { ...lifecycle, runCost, estimatedCostUsd: runCost.status === RUN_COST_STATUS.ESTIMATED ? runCost.estimatedCostUsd : null }

    // Terminal state must be durable before clients observe completion via polling.
    await commitJobRecord(record, { durable: true })

    // History archive must not fail the scan. Persist a non-blocking status for the UI.
    try {
      const archiveResult = await archiveSuccessfulScanHistory(record)
      record.historyArchive = {
        ok: Boolean(archiveResult?.ok),
        skipped: Boolean(archiveResult?.skipped),
        reasonCode: archiveResult?.reasonCode || null,
        idempotent: Boolean(archiveResult?.idempotent),
        runId: archiveResult?.runId || record.jobId,
      }
      await commitJobRecord(record, { durable: true })
    } catch (archiveError) {
      record.historyArchive = {
        ok: false,
        skipped: false,
        reasonCode: 'ARCHIVE_EXCEPTION',
        runId: record.jobId,
      }
      console.warn(
        JSON.stringify({
          event: 'scan_history_archive_exception',
          runId: record.jobId,
      ...
      try {
        ...
        // Non-blocking: scan already completed.
      }
    }
    ...
    queueRunTelemetryUpdate(
      ...
      buildRunTelemetryPatch({
        ...
        telemetryLogEntry: buildTelemetryLogEntry({
          telemetry: mergeTelemetryForLogEntry(record.telemetry, analysisResult, jobStartedAtMs, runCost),
          repository: {
