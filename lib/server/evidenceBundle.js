...
import { buildCapProvenanceRecords } from './coverageHonesty.js'

function artifactClassForPath(path) {
  const p = String(path || '').toLowerCase()
  if (
    /(^|\/)(package(-lock)?\.json|pnpm-lock\.yaml|yarn\.lock|go\.mod|go\.sum|cargo\.toml|cargo\.lock|pyproject\.toml|requirements\.txt|poetry\.lock)$/.test(
      p
    )
  ) {
  ...
  if (/\.(md|txt|adoc|rst)$/.test(p)) return 'documentation'
  if (/\.(ya?ml|json|toml|ini|xml|conf|cnf|properties)$/.test(p)) return 'config'
  if (/\.(pdf|docx?)$/.test(p)) return 'optional_documents'
  if (/\.(js|jsx|ts|tsx|py|go|java|rb|php|cs|swift|kt|rs|sql)$/.test(p)) return 'code'
  return 'other_text'
}

function countByArtifactClass(rows = []) {
  const out = {}
  for (const row of rows) {
    const path = row?.path
    if (!path) continue
    const klass = artifactClassForPath(path)
...
export function buildEvidenceBundle(repository, inventory, selection, orderedFiles, caps, flags = {}) {
  ...
  /** @type {string[]} */
  const truncatedByBytesPerFile = []
  /** @type {string[]} */
  const omittedByTotalBytes = []
  ...
  for (const { path, content } of orderedFiles) {
    ...
    if (excerpt.truncated) {
      ...
      truncatedByBytesPerFile.push(path)
    }
  ...
  const capRecords = buildCapProvenanceRecords({
    caps,
    coverage,
    capHits,
    affectedPathsByCode: {
      MAX_BYTES_PER_FILE: truncatedByBytesPerFile,
      MAX_TOTAL_BYTES_TO_MODEL: omittedByTotalBytes,
      MAX_FILES_FETCHED: coverage.maxFilesCapHit
        ? (selection.omitted || []).filter((o) => String(o?.reason || '').includes('cap')).map((o) => o.path)
        : [],
      MAX_REPO_TREE_ENTRIES: coverage.maxTreeSizeCapHit ? ['(repository tree truncated)'] : [],
    },
    impactByCode: {
      MAX_BYTES_PER_FILE: 'excerpt_truncated',
      MAX_TOTAL_BYTES_TO_MODEL: 'path_omitted',
      MAX_FILES_FETCHED: 'path_omitted',
      MAX_REPO_TREE_ENTRIES: 'path_omitted',
    },
  })
  coverage.capRecords = capRecords
    ...
    selection: {
      ...
      domainReservationByDomain:
        selection.domainReservationByDomain && typeof selection.domainReservationByDomain === 'object'
          ...
          : {},
      ...(selection.dimensionCoreEvidence
        ? { dimensionCoreEvidence: selection.dimensionCoreEvidence }
        : {}),
      ...(selection.protectedSecurityTargets
        ? {
            protectedSecurityTargets: selection.protectedSecurityTargets,
            protectedCoverageGap: !!selection.protectedCoverageGap,
          }
        : {}),
    },
    evidence,
  ...
  const selectedArtifactClassCounts = countByArtifactClass(selection.selected || [])
  const omittedArtifactClassCounts = countByArtifactClass(combinedOmitted || [])
  const evidenceArtifactClassCounts = countByArtifactClass(evidence || [])
  const rationaleCoverage = {
    selectedWithReasonCount: (selection.selected || []).filter((row) => String(row?.reason || '').trim().length > 0)
      .length,
    selectedTotalCount: (selection.selected || []).length,
  }
  ...
  const apiIngestion = {
    strategyVersion: STRATEGY_VERSION,
    totalFilesSeen: bundle.inventory.totalFilesSeen,
    totalEligibleFiles:
      (bundle.inventory.filesEligibleByTier?.tier1 || 0) +
      (bundle.inventory.filesEligibleByTier?.tier2 || 0) +
      (bundle.inventory.filesEligibleByTier?.tier3 || 0),
    nonGermaneExcludedCount: Math.max(
      0,
      (bundle.inventory.totalFilesSeen || 0) -
        ((bundle.inventory.filesEligibleByTier?.tier1 || 0) +
          (bundle.inventory.filesEligibleByTier?.tier2 || 0) +
          (bundle.inventory.filesEligibleByTier?.tier3 || 0))
    ),
    ...
    artifactClassCounts: {
      selected: selectedArtifactClassCounts,
      omitted: omittedArtifactClassCounts,
      evidence: evidenceArtifactClassCounts,
    },
    retrievalBreadth: {
      ...
      rationaleCoverage: {
        ...rationaleCoverage,
        percent:
          rationaleCoverage.selectedTotalCount > 0
            ? Number(((rationaleCoverage.selectedWithReasonCount / rationaleCoverage.selectedTotalCount) * 100).toFixed(2))
            : 0,
      },
    },
    protectedSecurityTargets: selection.protectedSecurityTargets ?? null,
    protectedCoverageGap: selection.protectedCoverageGap ?? false,
    dimensionCoreEvidence: selection.dimensionCoreEvidence ?? null,
  }
