...
import { writeContainedUtf8Artifact } from './advisoryArtifactWrite.js'
import {
...
function coverageForPass(bundle, repoData, promptTrimmedCount = 0) {
  return buildPassScopedCoverage(bundle?.coverage || {}, repoData?.ingestion || null, {
    promptTrimmedCount,
  })
}

function unmatchedCountForFamily(plan, family) {
  return (plan?.assignment?.unmatchedSurfacedByFamily?.[family] || []).length
}

function omittedQuotaCountForFamily(plan, family) {
  return (plan?.assignment?.omittedByQuota || []).filter((row) => row.family === family).length
}

function buildExtractionPrompt(userContent, pass, chunkIndex, chunkCount, adversarialBlock = '') {
  return `${userContent}

Pass focus (internal routing only — do not copy these labels into customer-facing claim text):
- Review id: ${pass.id}
- Security domain: ${pass.family}
- Analyze only this domain-specific file cluster.
...
- Customer-facing string fields (title, claim, specific_code_behavior, missing_control_or_unsafe_condition, impact) must use ordinary security-engineering English for a human reviewer. Do not write pipeline jargon into those fields (no evidence bundle, pass family, candidate admission, token budget, context window, artifact class, dimension ids, reason-code names, "grounded in scanned evidence", "downscoped", or "retained citations").
- Prefer observation → impact → next action in recommendation and finding claim text.
...
- For High/Critical findings include explicit attacker entry point, missing check, and security impact in the claim fields.
...
function writeQualityGateDebugArtifact(payload) {
  ...
  try {
    ...
    const safeId = String(payload?.correlationId || 'unknown').replace(/[^a-zA-Z0-9._-]+/g, '_').slice(0, 80)
    const outPath = writeContainedUtf8Artifact(outDir, `${safeId}.json`, JSON.stringify(payload, null, 2))
...
export async function analyzeSecurity(repoData, options = {}) {
  ...
  const plan = buildMultiPassPlan(bundle, {
    includePassFamilies: selectedPassFamilies,
    securitySurfacePlan: repoData.securitySurfacePlan || null,
    ingestionCaps: caps,
    applyQuotas: true,
  })
  ...
  const omittedByQuota = plan.assignment?.omittedByQuota || []
  const coverageHonestyCallout = buildCoverageHonestyCallout({
    unmatchedSurfacedPaths: plan.assignment?.unmatchedSurfacedPaths || [],
    residualPaths: plan.assignment?.residualPaths || [],
    omittedByQuotaCount: omittedByQuota.length,
    omittedByQuotaPaths: omittedByQuota.map((row) => row.path).filter(Boolean),
  })
  const ingestionCapRecords =
    Array.isArray(repoData?.ingestion?.capRecords) && repoData.ingestion.capRecords.length
      ? repoData.ingestion.capRecords
      : buildCapProvenanceRecords({
          caps,
  ...
  const capProvenanceNote = buildCapProvenanceCustomerNote(ingestionCapRecords)
  ...
  const requestContract = summarizeOpenAIChatCompletionRequestContract(selectedAnalysisModel)
  const baseDashboardTelemetry = {
    ...
    coverageHonestyCallout,
    capRecords: ingestionCapRecords,
    capProvenanceNote,
  }
  ...
  for (const pass of plan.passes) {
    // Residual supporting-context pass is visible in telemetry but skipped for OpenAI (Q4 / cost).
    if (pass.residual || pass.family === RESIDUAL_PASS_FAMILY) {
      passRuns.push({
        ok: true,
        passId: pass.id,
        family: pass.family,
        ...
        reasonCode: pass.reasonCode || 'UNMAPPED_SUPPORTING_CONTEXT',
      })
      continue
    }
    ...
    try {
      ...
      const promptTrimmed = extraction.promptBudgetTrimmedEvidenceCount || 0
      const passCoverage = coverageForPass(bundle, repoData, promptTrimmed)
      const passRun = {
        ok: true,
        passId: pass.id,
        family: pass.family,
        ...
        promptBudgetTrimmedEvidenceCount: promptTrimmed,
      ...
      if (dimensionId) {
        ...
        dimensionResults.set(
          ...
          assembleDimensionResult({
            ...
            promptTrimmedEvidenceCount: promptTrimmed,
            unmatchedSurfacedCount: unmatchedCountForFamily(plan, pass.family),
            omittedByQuotaCount: omittedQuotaCountForFamily(plan, pass.family),
            runtime: {
              progress: parsed.parseError || promptTrimmed > 0 ? 'partial' : 'completed',
      ...
      if (dimensionId) {
        ...
        dimensionResults.set(
          ...
          assembleDimensionResult({
            ...
            coverage: coverageForPass(bundle, repoData, 0),
            repoProfile,
          })
        )
        emitDashboard({ runState: 'running' })
        ...
        dimensionResults.set(
          ...
          assembleDimensionResult({
            ...
            runtime: {
              ...
              parseError: Boolean(mergedParsed.parseError),
            },
            coverage: coverageForPass(bundle, repoData, 0),
            repoProfile,
          })
  ...
  const failedPasses = passRuns.filter((p) => !p.ok && !p.residual)
  ...
  const successfulPasses = passRuns.filter((p) => p.ok && !p.residual && !p.skippedModel)
  ...
  // AQ9 — stamp model-examined paths onto allocation ledger from dimension results.
  if (Array.isArray(plan.assignment?.allocationLedger)) {
    for (const row of plan.assignment.allocationLedger) {
      const dim = dimensionResults.get(row.dimensionId)
      const examined = Array.isArray(dim?.evidence?.reviewedPaths)
        ? [...dim.evidence.reviewedPaths].filter(Boolean).sort((a, b) => a.localeCompare(b))
        : []
      row.modelExaminedPaths = examined
      row.zeroReviewReasonCode =
        examined.length === 0
          ? dim?.reasonCode || row.zeroReviewReasonCode || 'NO_PASS_EVIDENCE'
          : null
    }
    plan.assignment.lifecycleCounts = {
      ...(plan.assignment.lifecycleCounts || {}),
      reviewed: [...dimensionResults.values()].filter(
        (d) => (d?.evidence?.reviewedPaths || []).length > 0 || Number(d?.coverage?.reviewedFiles || 0) > 0
      ).length,
      completed: [...dimensionResults.values()].filter((d) => d.progress === 'completed').length,
    }
  }
  baseDashboardTelemetry.assignment = plan.assignment
  baseDashboardTelemetry.capRecords = ingestionCapRecords
  baseDashboardTelemetry.capProvenanceNote = capProvenanceNote
  ...
  emitDashboard({
    ...
    advisoryValidation: {
      ...
      humanRegisterWarnings: advisoryValidation.humanRegisterWarnings || [],
    },
    telemetry: {
