...
function buildExtractionPrompt(userContent, pass, chunkIndex, chunkCount) {
  return `${userContent}
...
async function runStructuredPassExtraction(client, pass, passBundle, repoData, supplementBlock = '') {
  const staticOverhead = `${ANALYSIS_SYSTEM_PROMPT}\n${buildExtractionPrompt('', pass, 998, 999)}${
...
function inferRepositoryProfile(repoData, bundle) {
  ...
  return inferRepoProfileFromPaths(paths, repoData?.language || '')
}
...
export async function analyzeSecurity(repoData, options = {}) {
  ...
  const baseDashboardTelemetry = { ingestion: repoData.ingestion || null }
  ...
  for (const pass of plan.passes) {
    ...
    try {
      ...
      const extraction = await runStructuredPassExtraction(client, pass, passBundle, repoData, '')
      ...
      if (dimensionId) {
        ...
        dimensionResults.set(
          ...
          assembleDimensionResult({
            ...
            runtime: {
              progress: parsed.parseError ? 'partial' : 'ready',
            },
  ...
  if (diligenceCorrectiveTelemetry.enabled) {
    ...
    for (const t of correctiveTargets) {
      ...
      try {
        ...
        const correctiveOut = await runStructuredPassExtraction(client, pass, passBundle, repoData, supplement)
        ...
        dimensionResults.set(
          ...
          assembleDimensionResult({
            ...
            promptTrimmedEvidenceCount: 0,
            runtime: {
              progress: mergedParsed.parseError ? 'partial' : 'ready',
            },
            coverage: buildPassScopedCoverage(),
            repoProfile,
          })
