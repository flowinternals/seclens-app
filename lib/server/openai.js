...
import { createDashboardPayload, createEmptyDimensionResult, DIMENSION_CATALOG } from '../shared/dimensions.js'
...
export async function analyzeSecurity(repoData, options = {}) {
  ...
  const dimensionResults = new Map(
    DIMENSION_CATALOG.map((dimension) => [dimension.id, createEmptyDimensionResult(dimension.id)])
  )
  ...
  for (const pass of plan.passes) {
    ...
    try {
      ...
      const promptPlan = preparePromptBoundedBundles(passBundle, repoData, {
        staticPromptOverhead: `${ANALYSIS_SYSTEM_PROMPT}\n${buildExtractionPrompt('', pass, 998, 999)}`,
      })
      const parsedClaims = []
      const chunkUsages = []
