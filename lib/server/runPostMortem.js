...
function addDeepOutputQualityAssertions(contract, dashboardDimensions, add) {
  ...
  for (const cd of contract.dimensions) {
    ...
    for (const r of recs) {
      ...
      if (body.length > 0 && !anchored && dimStatus === 'SUCCESS') {
        issues.push('recommendation_not_file_anchored')
        const recId = String(r.id || r.recommendationId || title || 'unnamed').slice(0, 80)
        add(
          `pm.output.dim.${dimId}.unanchored_recommendation`,
          'warn',
          'dimension_review',
          `${label}: unanchored recommendation identified (COVERAGE AQ10).`,
          `recommendationId=${recId}; hasReviewedPaths=${reviewed.length > 0}; files=${files.length}`
        )
      }
...
function describeDimensionFailure(reasonCode, lastError) {
  ...
  const table = {
    ...
    FILE_OMITTED_BY_CAP: {
      ...
      next: 'Use the retained evidence for immediate action, then prioritise omitted paths if the control is release-critical.',
    },
    STRUCTURED_PARSE_PARTIAL: {
      why: 'Structured extraction for this dimension was incomplete or failed to parse cleanly.',
      impact: 'Advisory artifacts may be incomplete even when some files were examined.',
      next: 'Inspect parse/admission telemetry and re-run the dimension with stable model settings.',
    },
    PROMPT_BUDGET_TRIM: {
      why: 'Evidence was trimmed to fit the model prompt budget.',
      impact: 'Not all assigned files were sent to the model for this dimension.',
      next: 'Reduce evidence size or raise prompt budget; review omitted excerpts.',
    },
    NO_PASS_EVIDENCE: {
      why: 'No evidence was assigned to this dimension for analysis.',
      impact: 'The dimension cannot claim security coverage for this run.',
      next: 'Confirm surface-plan membership and selection quotas for this dimension.',
    },
    OMITTED_BY_DIMENSION_QUOTA: {
      why: 'Per-dimension evidence quota omitted lower-ranked paths.',
      impact: 'Hotspot-scoped review; not full dimension surface coverage.',
      next: 'Review omitted-by-quota paths in assignment telemetry if deeper review is required.',
    },
    SELECTED_SURFACE_UNMATCHED: {
      why: 'Selected security-surface paths for this dimension were missing from analysis evidence.',
      impact: 'Dimension coverage is Incomplete relative to the planned security surface.',
      next: 'Inspect assignment.unmatchedSurfacedPaths and ingestion selection for this run.',
    },
...
export function buildRunPostMortem(run) {
  ...
  const assignment = telemetry?.assignment || dashboard?.telemetry?.assignment || null
  const unmatchedCount = Number(assignment?.surfacedPathsUnmatched || 0)
  const residualCount = Number(assignment?.residualPathCount || 0)
  if (unmatchedCount > 0 || residualCount > 0) {
    add(
      'pm.assignment_unmatched_or_residual',
      'warn',
      'file_coverage',
      'Selected paths were unmatched and/or routed as supporting context; not model-examined and not counted as dimension coverage.',
      `unmatched=${unmatchedCount}; residual=${residualCount}; source=${assignment?.source || '?'}`
    )
  } else if (assignment) {
    add(
      'pm.assignment_unmatched_or_residual',
      'pass',
      'file_coverage',
      'No unmatched surfaced paths or residual supporting-context evidence recorded.'
    )
  }

  const allocationLedger = assignment?.allocationLedger
  if (Array.isArray(allocationLedger) && allocationLedger.length > 0) {
    const zeroReview = allocationLedger.filter(
      (row) => Array.isArray(row.modelExaminedPaths) && row.modelExaminedPaths.length === 0 && row.zeroReviewReasonCode
    )
    add(
      'pm.allocation_ledger',
      zeroReview.length > 0 ? 'warn' : 'pass',
      'file_coverage',
      zeroReview.length > 0
        ? `Allocation ledger present; ${zeroReview.length} dimension(s) with zero model-examined paths.`
        : 'Allocation ledger present with model-examined paths for assigned dimensions.',
      zeroReview
        .slice(0, 8)
        .map((row) => `${row.dimensionId}:${row.zeroReviewReasonCode}`)
        .join(', ')
    )
  }

  const capRecords = (() => {
    if (safeArray(telemetry?.capRecords).length) return safeArray(telemetry.capRecords)
    if (safeArray(dashboard?.telemetry?.capRecords).length) return safeArray(dashboard.telemetry.capRecords)
    return safeArray(telIngest?.capRecords)
  })()
  if (capRecords.length > 0) {
    add(
      ...
      'warn',
      'file_coverage',
      'Effective numeric cap provenance recorded.',
      capRecords
        .map(
          (rec) =>
            `${rec.code}=${rec.resolvedLimitBytes ?? '?'} source=${rec.source} impact=${rec.impact} paths=${(rec.affectedPaths || []).length}`
        )
        .join('; ')
    )
  } else if (capHits.length > 0) {
    add(
      'pm.cap_provenance',
      'warn',
      'file_coverage',
      'Cap hits named without numeric provenance records (AQ8 gap).',
      capHits.join(', ')
    )
  }
  ...
  // --- Model usage ---
  const runCost =
    (run.runCost && typeof run.runCost === 'object' && run.runCost) ||
    (mus?.runCost && typeof mus.runCost === 'object' && mus.runCost) ||
    (telemetry?.runCost && typeof telemetry.runCost === 'object' && telemetry.runCost) ||
    (dashboard?.runCost && typeof dashboard.runCost === 'object' && dashboard.runCost) ||
    null
  ...
  if (runCost) {
    if (runCost.status === 'estimated' && typeof runCost.estimatedCostUsd === 'number') {
      add(
        'pm.run_cost',
        'pass',
        'advisory_quality',
        `Canonical runCost estimated US$${Number(runCost.estimatedCostUsd).toFixed(5)} for ${runCost.modelId}.`,
        `modelPriceVersion=${runCost.modelPriceVersion || '(missing)'}; basis=${runCost.basis || '(missing)'}`
      )
      if (run.analysisModel && runCost.modelId && run.analysisModel !== runCost.modelId) {
        add(
          'pm.run_cost_model_match',
          'fail',
          'advisory_quality',
          'runCost.modelId does not match run.analysisModel (cost provenance mismatch).',
          `runCost.modelId=${runCost.modelId}; analysisModel=${run.analysisModel}`
        )
      } else {
        add('pm.run_cost_model_match', 'pass', 'advisory_quality', 'runCost.modelId matches analysis model used.')
      }
      if (runCost.estimatedCostUsd === 0 && (runCost.inputTokens > 0 || runCost.outputTokens > 0)) {
        add(
          'pm.run_cost_zero_guard',
          'warn',
          'advisory_quality',
          'runCost shows $0.00 despite non-zero tokens — verify embedded rates.'
        )
      }
    } else if (runCost.status === 'unavailable') {
      add(
        'pm.run_cost',
        'pass',
        'advisory_quality',
        `Canonical runCost unavailable (${runCost.reasonCode || 'unspecified'}) — not represented as $0.00.`,
        `modelId=${runCost.modelId || '(none)'}`
      )
      if (runCost.estimatedCostUsd === 0) {
        add(
          'pm.run_cost_zero_guard',
          'fail',
          'advisory_quality',
          'Unavailable runCost must not set estimatedCostUsd to 0.'
        )
      }
    } else {
      add(
        ...
        'warn',
        'advisory_quality',
        `runCost present with unexpected status: ${runCost.status}`
      )
    }
  } else if (status === 'SUCCESS') {
    add(
      ...
      'warn',
      'advisory_quality',
      'SUCCESS run missing canonical runCost — treat as legacy/unavailable, not $0.00.'
    )
  }
  ...
  // DEFECT-OUTPUT-001 / plan Q6 — human-register on reconstructed customer-facing advisory output
  if (builtAdvisoryContract && typeof builtAdvisoryContract === 'object') {
    applyHumanReadableCoverageFallbacks(builtAdvisoryContract)
    const humanIssues = lintHumanReadableAdvisory(builtAdvisoryContract)
    const { errors: humanErrors, warnings: humanWarnings } = partitionHumanRegisterIssues(humanIssues)
    if (humanErrors.length > 0) {
      add(
        'pm.human_register',
        'fail',
        'contract_compliance',
        'AI-internal / pipeline register found in customer-facing advisory fields.',
        humanErrors.map((i) => `${i.code}@${i.path}`).join(', ')
      )
    } else if (humanWarnings.length > 0) {
      add(
        'pm.human_register',
        'warn',
        'contract_compliance',
        'Coverage notes still contained pipeline register after fallback pass (unexpected).',
        humanWarnings.map((i) => `${i.code}@${i.path}`).join(', ')
      )
    } else {
      add(
        'pm.human_register',
        'pass',
        'contract_compliance',
        'Customer-facing advisory fields use human-readable engineering register (telemetry-only fields not scored).',
      )
    }
  } else {
    add(
      ...
      'warn',
      'contract_compliance',
      'Skipped human-register check — no reconstructed advisory contract on this run.',
    )
  }

  // DEFECT-OUTPUT-001 — validate the customer export boundary. The PDF
  // renderer consumes this exact normalized plaintext, so this catches export
  // regressions that dashboard/advisory-contract checks cannot see.
  if (typeof run?.report === 'string' && run.report.trim()) {
    const pdfCheck = validatePdfExportText(run.report)
    if (!pdfCheck.valid) {
      add(
        'pm.pdf_export',
        'fail',
        'contract_compliance',
        'Rendered PDF export input contains customer-visible quality defects.',
        pdfCheck.issues.map((issue) => issue.code).join(', '),
      )
    } else {
      add(
        'pm.pdf_export',
        'pass',
        'contract_compliance',
        'PDF export input passed customer-language, coverage-disclaimer, and entity-leak checks.',
      )
    }
  } else {
    add(
      ...
      'warn',
      'contract_compliance',
      'Skipped PDF export check — no persisted report was available.',
    )
  }
...
function finalizeReport({
  ...
  return {
    ...
    runCost:
      (run?.runCost && typeof run.runCost === 'object' && run.runCost) ||
      (run?.modelUsageSummary?.runCost && typeof run.modelUsageSummary.runCost === 'object' && run.modelUsageSummary.runCost) ||
      (run?.telemetry?.runCost && typeof run.telemetry.runCost === 'object' && run.telemetry.runCost) ||
      (run?.dashboard?.runCost && typeof run.dashboard.runCost === 'object' && run.dashboard.runCost) ||
      null,
  }
}
