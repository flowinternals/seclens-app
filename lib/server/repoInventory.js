...
const TEXT_LIKE_EXT = /\.(txt|json|ya?ml|toml|md|mdx|graphql|properties|gradle|xml|plist|cnf|conf|ini|env)$/i
...
export function looksBinaryExtension(path) {
  ...
  if (/\.(png|jpe?g|gif|webp|ico|bmp|pdf|zip|tar|gz|tgz|7z|rar|exe|dll|so|dylib|woff2?|ttf|eot|mp3|mp4|webm|mov|sqlite|db|bin|pdf)$/i.test(base)) {
    return true
  }
